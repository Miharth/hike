...
//     $email = $_POST['email'];
//     $email = filter_var($_POST['email'], FILTER_SANITIZE_EMAIL); // Retrieve and sanitize 
//     $pass = $_POST['pass'];
//     $pass = htmlspecialchars(trim($_POST['pass']));
//     $cpass = $_POST['cpass'];
...
// NEW REGISTRATION LOGIC PASSWORD HASED //
...
if (isset($_POST['submits'])) 
{
    ...
    if ($pass != $cpass) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $query = "INSERT INTO `users` (name, email, password) VALUES ('$name', '$email', '$hashed_pass')";
            if (mysqli_query($conn, $query)) {
                echo "<script>alert('Registered successfully.');</script>";
